...
void Window::create(){
	...
	int n;
	if( graphName.size() < graphsInARow) n = 1;
	else{
	...
	int xn =  graphName.size() > this->graphsInARow ? graphsInARow : graphName.size();
	...
	for(int j = 0; j < n ; j++){
		for(int k = 0; k < xn; k++){
			...
			graph[i].width = xAdd,
			graph[i].height =  yAdd,
			graph[i].xOffset = xAdd * 0.1,
			graph[i].yOffset = yAdd * 0.1,
			graph[i].name = graphName[i];
			i++;
		}
	}
}
