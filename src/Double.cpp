...
Double::Double() : value(0.0), power(0) {}
...
Double::Double(double val, int pow) : value(val), power(pow) {}

Double::Double(const Double &o) : value(o.value), power(o.power) {}

Double Double::operator*(const Double &d) {
  return Double{value * d.value, power + d.power}.adapt();
}

Double BLL::Double::operator/(const Double &d) {
  Double ret{value / d.value, power - d.power};
  return ret.adapt();
}

Double Double::operator+(const Double &d) {
  Double ret{value + d.value * std::pow(10, d.power - power), power};
  return ret.adapt();
}
...
double Double::toStd() const { return value * std::pow(10, power); }
